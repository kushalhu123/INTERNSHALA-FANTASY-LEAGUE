#IMPORTING
import sqlite3
game_dbase = sqlite3.connect('cricketdata.db')
game_cursor = game_dbase.cursor()
# YOU ARE WRITING A FUNCTION TO GET THE POINTS
def getPoints(team):
    points = 0
    for g in team:
        for p in g:
            points += getVal(p[0])
    return points
#YOU ARE WRITING A FUNCTION TO GET THE VALUE OF A PLAYER
def getVal(player):
    query = 'SELECT Value FROM Stats WHERE Player = ?'
    game_cursor.execute(query, (player,))
    val =  game_cursor.fetchone()
    return val[0]
# YOU WRITING A FUNCTION TO GET THE TEAMSCORE
def getTeamScore(match, team):
    ...
    return player_score
#YOU ARE WRITING A FUNCTION TO GET THE PLAYERSCORE
